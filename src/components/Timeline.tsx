...
export default function Timeline({ data, onReset }: TimelineProps) {
  ...
  const calculatePosition = (date: Date, isStart: boolean) => {
    ...
    // For start dates, we want the position at the start of the day
    // For end dates, we want the position at the end of the day
    if (isStart) {
  ...
  return (
    <div>
      <div ref={timelineRef} className="bg-white p-8 rounded-lg shadow-lg">
        <div className="flex flex-col space-y-6">
          {/* Header */}
          <div className="grid grid-cols-[200px_1fr] items-center">
            <div className="text-sm font-medium text-gray-500">Phase/Goal</div>
            <div className="grid grid-cols-12 gap-0">
              {MONTHS.map((month) => (
                <div key={month} className="text-center text-sm font-medium text-gray-600">
                  {month}
                </div>
              ))}
            </div>
          ...
          {data.map((item, index) => (
            <div key={index} className="grid grid-cols-[200px_1fr] items-center">
              ...
              <div className="relative h-10">
                ...
                >
                  <div className="hidden group-hover:block absolute top-full mt-2 left-0 bg-white p-3 rounded shadow-lg z-10 min-w-[250px]">
